...
import { ServiceCollection, ServiceLifetime, resolveFromContext, tryResolveFromContext, createContainerMiddleware, } from "../src";
...
describe("Service lifetimes", () => {
    it("reuses singleton and disposes once", async () => {
        ...
        const a = provider.resolve("Singleton");
        const b = provider.resolve("Singleton");
        expect(a).toBe(b);
    ...
    it("caches scoped per scope", () => {
        ...
        const a1 = scope1.resolve("Scoped");
        const a2 = scope1.resolve("Scoped");
        const b1 = scope2.resolve("Scoped");
        expect(a1).toBe(a2);
    ...
    it("creates transient every time", () => {
        ...
        const provider = services.build();
        const a = provider.resolve("Transient");
        const b = provider.resolve("Transient");
        expect(a).not.toBe(b);
    });
});
...
describe("Hono helpers", () => {
    it("attaches scope per request and resolves/tries correctly", async () => {
        ...
        const next = vi.fn(async () => {
            const container = ctx.var.container;
            container.resolve("Disposable");
        });
        await middleware(ctx, next);
        expect(next).toHaveBeenCalled();
        const requestId = resolveFromContext(ctx, "RequestId");
        expect(requestId).toMatch(/^req-/);
        const missing = tryResolveFromContext(ctx, "Nope");
        expect(missing).toBeUndefined();
        expect(dispose).toHaveBeenCalledTimes(1);
    });
    it("returns undefined when container is missing", () => {
        const ctx = { var: {}, set: () => { } };
        expect(tryResolveFromContext(ctx, "Anything")).toBeUndefined();
    });
    it("throws when resolving without container", () => {
        const ctx = { var: {}, set: () => { } };
        expect(() => resolveFromContext(ctx, "X")).toThrow();
    });
    it("supports custom variable name", async () => {
        ...
        const ctx = { var: {}, set: (k, v) => (ctx.var[k] = v) };
        await middleware(ctx, async () => { });
        const value = resolveFromContext(ctx, "Value", "scope");
...
describe("Internals and guards", () => {
    it("wraps singleton factories and exposes descriptors", () => {
        ...
        expect(provider.resolve("Factory").n).toBe(1);
    });
    it("validates scoped descriptor in getOrCreate", () => {
        ...
        expect(() => scope.getOrCreate("X", descriptor)).toThrow();
    });
